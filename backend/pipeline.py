...
async def analyze_sentiment(chunks: list[str]) -> dict:
    financial_keywords = [
...
Respond ONLY in this exact JSON format:
{
  ...
  "key_highlights": [
    "specific highlight with exact numbers e.g. Net profit surged 32% YoY to Rs 2,252 crore",
...
# ── Now async — awaits sentiment directly ──
async def run_pipeline(company: str, transcript: str) -> dict:
    logger.info(f"Running pipeline for: {company}")
    chunks = chunk_transcript(transcript)
    sentiment = await analyze_sentiment(chunks)  # await directly
